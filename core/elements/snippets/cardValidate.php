<?php
    ...
    if (!empty($vnumber)){
        if ($luhn->validate(substr($vnumber, 0, -1), substr($vnumber, -1, 1)) == true) {
            ...
            $vresult = "Операция завершилась с ошибкой! Ошибка: неверно введена карта или карты не существует!<br>Окно закроется автоматически, через несколько секунд.";
        }
        ...
        if ($_GET['status'] == "canceled") {
            if (!empty($reason)) {
                $errorReason = "<br>Ошибка: {$reasonValue}";
            }
            echo "Операция завершилась с ошибкой!{$errorReason}<br>Окно закроется автоматически, через несколько секунд.";
        } else {
            echo "Операция успешно завершена! Окно закроется автоматически, через несколько секунд.";
        }
    }